...
def check_cart(request, product, product_weight):
    try:
        cart = AddToCart.objects.get(
            Q(product=product) & Q(user=request.user) & Q(product_weight=product_weight))
        return True
...
def get_cart_amt_detail(request):
    if request.user.is_authenticated:
        ...
        if cart:
            for item in cart:
                if not item.product.soft_delete and item.product.status:
                    returnData["cartAmt"] += decimal.Decimal(item.quantity * (item.product_weight.price-item.product_weight.discount_price))
                    returnData["cartTaxAmt"] +=decimal.Decimal((item.quantity*(item.product_weight.price-item.product_weight.discount_price)))*item.product.tax/100
